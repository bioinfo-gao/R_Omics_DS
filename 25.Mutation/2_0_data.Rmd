<!--
来源说明: 本文件是 maftools 官方 vignette 的原样拷贝(YAML 的 author 为 Anand Mayakonda),
不是本项目自写的分析脚本。留在这里作为 maftools 用法参考。

⚠ 因此不要按本项目的规范去改写它 —— 上游更新时直接替换整份更合适。
本目录中属于你自己的分析是 2_1_MAF_summary.Rmd(该文件已有较完整的中文注释)。
-->

<!-- ⚠ 另注: 本文件与同目录 2_2_MAF_oncoplot.Rmd 的 YAML 标题同为 "Customizing oncoplots",
     两者很可能是同一份 vignette 的不同版本, 使用前先确认要以哪一份为准。 -->
...
library(psych)
library(R.utils)
# gzip("C:/Users/zhen-/AppData/Local/R/win-library/4.5/maftools/extdata/tcga_laml.maf") 
# gunzip("C:/Users/zhen-/AppData/Local/R/win-library/4.5/maftools/extdata/tcga_laml.maf.gz") 
# con <- gzfile("C:/Users/zhen-/AppData/Local/R/win-library/4.5/maftools/extdata/tcga_laml.maf.gz", "rt") # "rt" for read text mode
# # Read the data (e.g., as a CSV)
...
```{r setup, message=FALSE, warning=FALSE}
lamf=read.table("C:/Users/zhen-/AppData/Local/R/win-library/4.5/maftools/extdata/tcga_laml.maf/tcga_laml.maf",sep = "\t",header = T)
...
## Changing colors for variant classifications , this one is very elgent  

```{r}
#One can use any colors, here in this example color palette from RColorBrewer package is used
vc_cols = RColorBrewer::brewer.pal(n = 8, name = 'Paired')           # << ==== change colors here 
names(vc_cols) = c(
  'Frame_Shift_Del',
  'Missense_Mutation',
  'Nonsense_Mutation',
...
#oncoplot(maf = laml, colors = vc_cols, top = 10)                    # << ==== change colors here 
oncoplot(maf = laml,, draw_titv = TRUE,  colors = vc_cols, top = 10) # << ==== change colors here 
```
...
GISTIC (Genomic Identification of Significant Targets in Cancer) 是一个由 Broad 研究所开发的著名工具。

核心功能： 它的唯一目的是分析拷贝数变异 (Copy Number Variations, CNVs) 数据。

工作原理： GISTIC 会分析一大批肿瘤样本（比如来自 TCGA 的数据），以识别在统计上显著的、反复出现的基因组区域扩增 (Amplification) 或缺失 (Deletion)。

G-score： 它为每个基因组区域计算一个 “G-score”，该分数同时考虑了变异的频率（在多少样本中出现）和幅度（拷贝数变化有多大）。

最终输出： GISTIC 的最终输出是一些表格文件，列出了显著扩增或缺失的“峰值”区域（peaks），这些区域被认为是驱动癌症发生的潜在区域（包含癌基因或抑癌基因）。

ZG 思考： 这个看起来是对一大批肿瘤样本进行分析，然后输出一些峰值区域，这些区域可能是驱动癌症发生的潜在区域。
不是单个病人，可以参考下面生产的图的标题
...
### Custom copy-number table <font color='brown'> (CN : Copy Number)
</font>
