...
> 🛑 **本仓库有三个 Perl 脚本自 2019 年起静默失效，另有两处已产出结果需作废重跑。**
> 详见 [README.md](README.md) 顶部，或本文件「第 ⑪–⑫ 轮」小节。
...
| 2026-09-18 ⑧ | `e45a2b3` | 还原 17 行被吞代码 + 修 6 处 + 注释 18 个 | 18 个文件 | **是** |
| 2026-09-18 ⑨ | `5183292` | 修 1 处 + 注释 5 个 | 5 个文件 | **是** |
| 2026-09-18 ⑩ | `a4ccc9d` | 修 4 处 + 注释 2 个 | 2 个文件 | **是** |
...
## 第 ⑧–⑩ 轮 —— 主导发现: 一种把代码吞进注释的破坏模式

一次有损编码转换把**注释行与紧随其后的代码行并成了一行**, 于是可运行的语句整个落进注释里。
做成定向扫描后抓全了全仓库, 共 **17 行**, 集中在四个文件:

| 文件 | 被吞行数 | 后果 |
| :--- | :---: | :---: |
| `17.Imm_checkpoint` | 6 | `group`/`x` 未定义; `write.table`/`pdf` 消失 ⇒ **整脚本不产生任何输出**; `dev.off()` 关错设备 |
| `18.multiple_group_difference` | 6 | `groups`/`samSample`/`gene`/`group`/`boxplot`/`pdf` 全丢 |
| `22.Immu_cell_state_with_Survival` | 2（另加此前 2 处） | `outTab`/`res3` 未定义 |
| `02.DE_limma/limma.R` | 2 | 变量名 `diffLab` 被从中间拆成两行 ⇒ 该变量从未存在 |

每处还原都在**原地注明判据**（下一行引用了哪个变量），多处在兄弟脚本里有逐字相同的对照行。

### 另有一类乱码: GBK 字节恰好也是合法 UTF-8

例如 `图片` 的 GBK 编码 `CD BC C6 AC` 解成 UTF-8 是 `ͼƬ`（希腊/拉丁扩展字母），
因此不会触发任何解码错误，前几轮的编码修复整类漏掉。全仓库扫描得 **33 处，全部位于注释文本内部**
（含行尾注释），**没有一处影响代码**。它们是已不可还原的乱码注释里侥幸存活的孤字，
强行 GBK 解码只会得到"叶斯""圆圈"这样的碎片，故**不做修复**。

## 第 ⑧–⑩ 轮修复的其他缺陷

| 文件 | 问题 | 判据 |
| :--- | :---: | :---: |
| `07.CoExpression` | `outTab` 未初始化 + 一段坏掉的 `future_lapply` 抢在前面执行 ⇒ **整脚本跑不起来** | 实测 `rbind` 报 object not found、`$` 对矩阵报错 |
| `22` `80` | `plan()` 来自 future，`future.apply` **不重导出**它 | 实测 `getNamespaceExports` |
| `54` `55` | 形参默认值写成 R 中不存在的 `null` | — |
| `57.Multi_GEO_Merge` | `str_detect` 模式串里有**字面制表符** ⇒ 第三个数据集全部样本误判为 "N" | `cat -A` 显示 `^I` |
| `02.DE_limma` | `pdf(width=8000)` 把英寸当像素；`makeContrasts` 方向与全仓库其余脚本相反 | pdf width 单位为英寸 |

## 只标不改的（判据与建议改法都写在代码注释里）

| 文件 | 问题 |
| :--- | :---: |
| `31.ML_Modeling` | 上游 `28.lasso` 把样本名裁成 4 段，此处按 5 段取组别 ⇒ **con/treat 全为 0**，神经网络在全零标签上训练（已实测） |
| `27.cu` | `sampleType` 按「正常在前」构造，但本脚本从不重排列 |
| `52.oncoPredict` | 表达矩阵与药敏矩阵**按位置配对**，无样本名对齐（隔壁 `50` 有 `intersect`） |
| `63.GEO_MultiDis` | `~0+group` 无截距设计 + `coef=2` ⇒ 检验"第 2 组均值是否为 0" |
| `83.ComBat` | TCGA 肿瘤 + GTEx 正常，批次与生物学分组**完全共线**，`mod=~1` 会抹掉真实差异 |
| `68.Multi_GEO_Batch_Removal` | `batchType`/`modType` 是**手写死的样本计数**，与矩阵无名字层面关联；两行"方法二选一"实际只有后一行生效；`removeBatchEffect` 未传 `design=` |
| `57.Multi_GEO_Merge` | 两种芯片平台的三个数据集直接 `cbind`，无批次校正 |
| `80.Wilcox` | TPM 喂给 edgeR 的 TMM 流程 |
| `02` vs `13`/`14` | 同一文件结构，一个从第 3 列取表达值、另两个从第 2 列取，不可能都对 |
